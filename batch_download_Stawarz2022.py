...
if __name__ == "__main__":
    ...
    interval_list = pd.read_csv(os.path.join(data_dir, 'turbulence_intervals_Stawarz2022.csv'), names=['start_time', 'end_time'])
    ...
    for i in range(len(interval_list)):
        ...
        try:
            start_time = interval_list['start_time'][i]
            end_time = interval_list['end_time'][i]
            ...
            write_data(data, trange, os.path.join(data_dir, 'Stawarz2022_reconnection'))

            print(rf"Downloaded data for {trange[0]} - {trange[1]}")
        
        except:
            print(rf"Download failed for {trange[0]} - {trange[1]}")
            failures.append(trange)

fail_df = pd.DataFrame(data=failures)
fail_df.to_csv(os.path.join(data_dir, 'failed_downloads_Stawarz2022.csv'))